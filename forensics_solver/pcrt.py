...
def str2hex(s):
    if type(s)==str:
	    return hexlify(s.encode()).decode().upper()
    else:
...
def ReadFile(filename):
	try:
		...
		##print(Termcolor('Error',e+': '+filename))
		return -1
	return data
...
class PNG(object):
	...
	def PrintPicInfo(self):
		...
		+'ColorType: %s'%(mode_dict[self.mode])
	...
	def CheckFormat(self,data):
		...
		status = [True for p in png_feature if p in data.decode(errors='ignore')]
		if status == []:
			return -1
		return 0
	...
	def CheckHeader(self,data):
		...
		if str2hex(Header)!='89504E470D0A1A0A':
			...
			if choice == 'y' or choice == '':
				Header=unhexlify(b"89504E470D0A1A0A")
	...
	def CheckIDAT(self,data):
		...
		IDAT_table = []
		idat_begin = data.find(unhexlify(b'49444154'))-4
		if idat_begin == -1:
		...
		if pos == -1:
			...
			if data[pos+8:] != '':
				...
				if choice == '1':
					...
					#print('hex(data):',hexlify(data[pos+8:]))
					pass
...
def PCRT(challenge,file,outfile):
